...
namespace UI
{
    public partial class BlanketPurchaseAgreement : Form
    {
        private readonly Connection_class BPAConn = new Connection_class();

        public BlanketPurchaseAgreement()
        {
            InitializeComponent();
        }

        private void BlanketPurchaseAgreement_Load(object sender, EventArgs e)
        {
        ...
        private void GenerateNextContractNumber()
        {
            try
            {
                using (var conn = new MySqlConnection(BPAConn.conndb.ConnectionString))
                {
                    conn.Open();

                    // Retrieve the latest ContractNumber from the database
                    string query = "SELECT MAX(CONVERT(SUBSTRING(ContractNumber, 4), UNSIGNED INTEGER)) AS MaxContractNumber FROM bpa";
                    using (var command = new MySqlCommand(query, conn))
                    {
                        ...
                        // Generate the next ContractNumber with the format "bpa000X"
                        string nextContractNumberString = "bpa" + nextContractNumber.ToString("0000");
        ...
        private void PopulateSupplierComboBox()
        {
            try
            {
                ...
                using (MySqlConnection conn = BPAConn.conndb)
                {
                    conn.Open();
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            ...
            if(cbItemName.SelectedIndex == -1 || cbSupplierName.SelectedIndex == -1 || tbQty.Text == "" || tbContactNumber.Text == "" || tbUOM.Text == "" || tbPrice.Text == "" || tbDiscount.Text == "" || tbTermsCondition.Text == "" || tbSignature.Text == "")
            {
            ...
            DialogResult result = MessageBox.Show($"Are you sure you want to create BPA agreement?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                try
                {
                    using (var conn = new MySqlConnection(BPAConn.conndb.ConnectionString))
                    {
                        ...
                        string query = @"INSERT INTO bpa (ContractNumber, StartDate, EndDate, EffectiveDay, SupplierID, ItemID, Price, Quantity, UOM, Discounts, TermsAndCondition, Status, Signatures)
                                 VALUES (@ContractNumber, @StartDate, @EndDate,@EffectiveDay, @SupplierID, @ItemID, @Price, @Quantity, @UOM, @Discounts, @TermsAndCondition, @Status, @Signatures)";
                        ...
                        using (var command = new MySqlCommand(query, conn))
                        {
                            ...
                            command.Parameters.AddWithValue("@Discounts", Convert.ToInt32(tbDiscount.Text));
                            ...
                            MessageBox.Show("BPA agreement created successfully.");
                        }
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Error creating BPA agreement: " + ex.Message);
                }
            }
        }
