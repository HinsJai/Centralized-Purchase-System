...
namespace UI
{
    public partial class PPADetails : Form
    {
        private readonly Connection_class PPADetailConn = new Connection_class();
        private string contractNumber;

        public PPADetails(string contractNumber)
        {
        ...
        private void PPADetails_Load(object sender, EventArgs e)
        {
            setDefaultBuyerInfo();
            setContratDetails();
        ...
        private void setContratDetails()
        {
            try
            {
                string query = "SELECT ppa.*, item.itemName, supplier.* " +
                               "FROM ppa JOIN item ON ppa.ItemID = item.itemID " +
                               "JOIN supplier ON ppa.SupplierID = supplier.supplierID " +
                               "WHERE ppa.ContractNumber = @contractNumber";

                using (MySqlConnection connection = new MySqlConnection(PPADetailConn.conndb.ConnectionString))
                {
                    connection.Open();
                    ...
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        ...
                        using (MySqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                tbTentativeSchedule.Text = reader["TentativeSchedules"].ToString();
        ...
        private void PopulateSupplierComboBox()
        {
            try
            {
                ...
                using (MySqlConnection conn = PPADetailConn.conndb)
                {
                    conn.Open();
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            tbTentativeSchedule.ReadOnly = false;
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            tbTentativeSchedule.ReadOnly = true;
        ...
        {
            DialogResult result = MessageBox.Show($"Are you sure you want to delete PPA: {tbCNumber.Text}?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                try
                {
                    string deleteQuery = "DELETE FROM ppa WHERE ContractNumber = @contractNumber";
                    ...
                    using (MySqlConnection connection = new MySqlConnection(PPADetailConn.conndb.ConnectionString))
                    {
                        ...
                        using (MySqlCommand command = new MySqlCommand(deleteQuery, connection))
                        {
                            ...
                            if (rowsAffected > 0)
                            {
                                MessageBox.Show("PPA deleted successfully.");
                                this.Close();
                            }
                            else
                            {
                                MessageBox.Show("Failed to delete PPA.");
                            }
                        }
                    }
                }
        ...
        {
            if (cbItemName.SelectedIndex == -1 || cbItemName.SelectedIndex == -1 || tbQty.Text == "" || tbContactNumber.Text == "" || tbUOM.Text == "" || tbPrice.Text == "" || tbTentativeSchedule.Text == "" || tbTermsCondition.Text == "" || tbSignature.Text == "")
            {
            ...
            DialogResult result = MessageBox.Show($"Are you sure modify PPA : {tbCNumber.Text}?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                try
                {
                    string updateQuery = "UPDATE ppa " +
                    ...
                    using (MySqlConnection connection = new MySqlConnection(PPADetailConn.conndb.ConnectionString))
                    {
                        ...
                        using (MySqlCommand command = new MySqlCommand(updateQuery, connection))
                        {
                            ...
                            command.Parameters.AddWithValue("@tentativeSchedules", tbTentativeSchedule.Text);
                            ...
                            if (rowsAffected > 0)
                            {
                                MessageBox.Show("PPA details updated successfully.");
                                this.Close();
                            }
                            else
                            {
                                MessageBox.Show("Failed to update PPA details.");
                            }
                        }
                    }
                }
