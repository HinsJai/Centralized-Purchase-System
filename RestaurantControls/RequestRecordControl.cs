...
{
    public partial class RequestRecordControl : UserControl
    {
        ...
        private dvgRequestRecordDetails requestDetailsForm;

        public RequestRecordControl()
        {
            InitializeComponent();
        }

        private void RequestRecordControl_Load(object sender, EventArgs e)
        {
            SetTable();
        ...
        private void dgvRecordList_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (dgvRecordList.SelectedCells.Count > 0) // Make sure a valid row is clicked
                {
                    ...
                    string qty = selectedRow.Cells[11].Value.ToString();
                    string remark = selectedRow.Cells[12].Value.ToString();


                    // Create a new instance of RequestDetailsForm if it doesn't exist
                    if (requestDetailsForm == null || requestDetailsForm.IsDisposed)
                    {
                        requestDetailsForm = new dvgRequestRecordDetails();
                    }

                    // Set the selected data in the RequestDetailsForm
                    requestDetailsForm.SetData(requestID, requestItemVID, requestItemName, requestItemTypeName, requestItemCategory, requestBrand, requestQty, unit, pickUpDate, requestDateTime, status, remark, qty);


                    // Set the index of cbItemName to 0
                    requestDetailsForm.SetComboBoxIndex(0);
